(function () {
  // The banner is the base templates' own div, emitted only where
  // site.json sets an announcement; every other page loads this
  // script too, and leaves at the guard.
  var banner = document.querySelector(".announcement");
  if (!banner) return;
  var source = banner.getAttribute("data-source");
  var shown = null;
  function dismissed(html) {
    try { return localStorage.getItem("announcement-dismissed") === html; }
    catch (e) { return false; }
  }
  function render(html) {
    html = (html || "").trim();
    if (html === shown) return;
    if (!html || dismissed(html)) {
      banner.hidden = true;
      banner.innerHTML = "";
      shown = null;
      return;
    }
    // the announcement rides in a wrapper of its own, as it does in
    // jupyter.org's .global-banner-content: the banner's own box keeps
    // the smaller measure its padding and dismiss button are drawn
    // against, and the wrapper sets the announcement's own size (see
    // .announcement-content), whether the source gives markup or text
    banner.innerHTML = "";
    var content = document.createElement("div");
    content.className = "announcement-content";
    content.innerHTML = html;
    banner.appendChild(content);
    var close = document.createElement("button");
    close.type = "button";
    close.className = "announcement-close";
    close.setAttribute("aria-label", "Dismiss announcement");
    close.textContent = "×";
    close.addEventListener("click", function () {
      banner.hidden = true;
      try { localStorage.setItem("announcement-dismissed", html); } catch (e) {}
    });
    banner.appendChild(close);
    banner.hidden = false;
    shown = html;
  }
  if (/^https?:\/\//i.test(source)) {
    // shared/announcement-init.html, inline in the head, has already
    // painted this same content before the header was parsed, so
    // navigating the site doesn't jerk the page when the banner
    // arrives. Rendering it again here is what adds the dismiss
    // button (absolutely positioned, so it shifts nothing) and gives
    // the fetch below its `shown` to compare against; the fetch then
    // corrects the rare announcement that changed since.
    var cached = null;
    try { cached = JSON.parse(localStorage.getItem("announcement-cache")); }
    catch (e) {}
    if (cached && cached.src === source) render(cached.html);
    fetch(source).then(function (r) { return r.ok ? r.text() : ""; })
      .then(function (html) {
        try {
          localStorage.setItem("announcement-cache",
                               JSON.stringify({src: source, html: html}));
        } catch (e) {}
        render(html);
      }, function () {});
  } else {
    render(source);
  }
})();
