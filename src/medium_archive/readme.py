...
* `../site/site.toml` is **hand-written**, and is the one input the
...
```
../site/site.toml             hand-written site settings read by every site
...
hand-written `site/site.toml`, the one exporter input that is not part
...
the site is built, pelican's is `site.toml` beside the `pelicanconf.py`
...
site-pelican/                   `medium-archive hugo|pelican`: the same
  ...
  config/_default/            (hugo) the site config as a directory:
    params.toml                 what the pages say about themselves --
                                ...
                                site/site.toml, editable by hand, and
                                documented key by key in the file itself
  ...
  site.toml                   (pelican) the same data for the pelican
...
* The images beside each page are display copies, capped per
  `site/site.toml` (the full-resolution originals stay in
...
* What the hugo and pelican sites *serve* those redirects as is
  `site/site.toml`'s `redirects`: `"stubs"` (a meta-refresh page at every
...
* The `data/*.json` files in `site-hugo/` and `site-pelican/`, and
  ...
  `site.toml`), are derived from the archive and from `site/site.toml`
  ...
  site-wide setting in `site/site.toml`; editing the copies directly is
...
Before deploying, set `base_url` in `site/site.toml` to the site's real
