...
# Companion plugin written into site/ beside myst.yml. myst-listing renders
# each gallery card's `thumbnail` as a CSS background-image, passing the
# front-matter string through verbatim -- fine for absolute URLs, broken for
# a thumbnail beside the post, because nothing serves the source tree and
# mystmd's image pipeline never looks inside CSS backgrounds. This transform
# (which must be listed *after* myst-listing in myst.yml: plugin transforms
# run in listing order) rewrites each card's cover into a real image node
# with a listing-page-relative path, so mystmd copies the file into the
# build and rewrites the URL like any post body image.
...
def build_site(out: Path) -> Path:
    ...
    (site / COVER_SHIM_NAME).write_text(COVER_SHIM, encoding="utf-8")
