"""The myst step: build a MyST (mystmd) site in site-myst/ from the
...
Works from archive/posts.json and archive/posts/ alone (run convert
first), so the site is as reproducible as the posts are: raw/ + fixups/
-> convert -> posts/ -> myst -> site-myst/. Never touches the network;
rendering the site (`myst start` or `myst build --html` inside
site-myst/) is mystmd's job, not this step's. (The landing-page gallery
uses the myst-listing plugin, which mystmd downloads at build time like
the site theme itself.)
...
optional hand-written site/site.toml, so it is versioned with the archive
...
def write_myst_yml(site: Path, manifest: dict, stems: dict, config: dict):
    lines = ["# Generated by `medium-archive myst`; do not edit.",
             "# Title and description come from site/site.toml.",
             "version: 1",
             "project:",
    ...
    lines += ["  plugins:",
              "    # the gallery plugin, then the transform that fixes its",
              "    # cover images (order matters)",
